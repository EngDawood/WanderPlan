...
### 1. Home (`/`)
The landing page of the application. It features a welcoming hero section with a search bar where users can type in a destination city to start planning their trip. It also includes quick-access chips for popular destinations, recent searches, and a language toggle (English/Arabic).

### 2. City Search / Generation (`/search`)
Once a user selects a city, they are taken to this screen to finalize their trip details. It includes a search input with autocomplete, recent searches, and popular destinations.

### 3. Preferences (`/preferences`)
Users select the types of places they want to visit (e.g., Restaurants, Cafes, Attractions). The screen features a scrollable list of categories with a fixed "Find Places" button at the bottom.

### 4. Places List (`/places`)
Displays a list of places based on the user's selected city and preferences.
- Users can toggle between List, Grid, and Compact view modes.
- List view includes additional details like opening hours and a top review.
- Users must select at least 3 places to proceed.
- A floating button at the bottom triggers the AI itinerary generation.

### 5. Generated Itinerary (`/itinerary`)
...
### 8. Saved Itinerary Detail (`/saved/:id`)
...
### 11. Sign In (`/signin`)
Allows existing users to log into their accounts using their email and password.

### 12. Sign Up (`/signup`)
Allows new users to create an account by providing their name, email, and password.

### 13. Forgot Password (`/forgot-password`)
Allows users to request a password reset email if they have forgotten their password.

### 14. Change Password (`/change-password`)
